...
class PokerHand {
  ...
  getHeroActionPreflop = async () => {
    ...
    if(heroCards.length == 2) {
      if (origHeroAction !== this.heroPreflopAction) {
        ...
        const unicodeHeroCards = heroCards.map((card) => unicodeCards?.[card] || card)
        $(`#hud_advice_${table_id}`).html(`<span class="hud-pre-flop-advice">Advice: ${this.heroPreflopAction} your  <span class="unicode-cards">${unicodeHeroCards.join('')}</span></span>`)
      }
      ...
      if (['39', '18', '19', '7', '6', '5', '21', '4', '10', '4', '10','8'].includes(dealerMessageCode.toString())) {
        ...
        if (actionSeat) {
          ...
          if(playerAction === 'Fold') {
            ...
            $(`tr#tr_${tableId}_${playerName}`).hide()
          }
  ...
  initSeats = async (arrayOfSeats) => {
    ...
    if (!this.seatsInitialized) {
      const tableId = this.database_model.table_name
      $(`#hud_stats_footer_${tableId}`).remove()
      ...
      $.get(apiEndpoint, (response) => {
        ...
        response.hud_data.forEach((dataRow) => {
          ...
          const ag = dataRow['AG']
          let trClass = hands < 200 ? 'player-row low-sample-size' : 'player-row'
          if(hands >= 1000) {
            trClass  += " high-sample-size"
          }
          const htmlRow = `<tr class="${trClass}" id="tr_${tableId}_${player}"><td>${name}</td><td>${hands}</td><td>${vpip}</td><td>${pfr}</td><td>${threeBet}</td><td>${wwsf}</td><td>${wtsd}</td><td>${wsd}</td><td>${ag}</td></tr>`
      ...
      const hudFooter = `<div class="hud hud-footer" id="hud_stats_footer_${tableId}">
                          <a class="hud hud-footer">Show Folded Players</a>
                        </div>`
      $(hudFooter).appendTo($(`#hud_${tableId}`))
      $(`#hud_stats_footer_${tableId}`).click(() => {
        $(`table#hud_stats_table_${tableId}`).find('tr').show()
        $(`table#hud_stats_table_${tableId}`).find('tr').attr('id',`tr_${tableId}`)
        $(`#hud_stats_footer_${tableId}`).hide()
      })
      ...
      case 'TableHandMuckedCards': {
        ...
        if (hand) {
          ...
          if(OW.HERO_NAME === 'cornbl4ster') {
            ...
            $(`div#hud_muck_${table_id}`).html(`<span class="hud-muck-advice">${playerName} mucked <span class="unicode-cards">${muckedUnicodeCards.join('')}</span> with <span class="unicode-cards">[${unicodeBoard.join('')}]</span> on the board.</span>`)
          }
