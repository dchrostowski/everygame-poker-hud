  ...
  getHeroActionPreflop = async () => {
    ...
    if (heroCards.length == 2) {
      if (origHeroAction !== this.heroPreflopAction) {
        ...
        $(`#hud_advice_${table_id}`).html(`<span class="hud-pre-flop-advice">Advice: ${this.heroPreflopAction} your  <span class="unicode-cards">${heroCards.join(' ')}</span></span>`)
      }
      ...
      case 'TableHandMuckedCards': {
        ...
        if (hand) {
          ...
          if (OW.HERO_NAME === 'cornbl4ster') {
            ...
            $(`div#hud_muck_${table_id}`).html(`<span class="hud-muck-advice">${playerName} mucked <span class="unicode-cards">${playerCards.join(' ')}</span> with <span class="unicode-cards">[${playerCards.join(' ')}]</span> on the board.</span>`)
          }
