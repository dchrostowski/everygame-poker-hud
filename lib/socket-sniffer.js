...
class PokerHand {
  constructor(site_name, handId, socketId, tableInfo) {
    ...
    this.heroPreflopAction = 'decide yourself'
  ...
  updateStreet = async (newStreet) => {
    ...
    if(newStreet !== 'PreFlop' && newStreet !== 'HoleCards') {
      $(`#${table_id}_advice`).hide()
    }
    else {
      $(`#${table_id}_advice`).show()
      $('.helper').show()
    }
  }
  ...
  assignPositionsToSeats = async (seats, dealerPos) => {
    ...
    if (dealerSeat?.name === this.heroName) {
      ...
      OW.LAST_HERO_POSITION[this.database_model.table_name] = 'BTN'
    }
  ...
  getHeroActionPreflop = async () => {
    ...
    if(heroCards.length == 2) {
      if (origHeroAction !== this.heroPreflopAction) {
        console.log('Table ' + this.database_model.table_name + ": " + this.heroPreflopAction + ' your ' + heroCards.join(' '))
        ...
        // if(this.heroPreflopAction === 'Fold') {
        //   console.log('simualte click now')
        //   const x = gameCanvas.width -20
        ...
        //     var mousedown = new $.Event("click");
        //     $(gameCanvas).trigger(mousemove)
        //     $(gameCanvas).trigger(mousedown)

        //   }

        //   setTimeout(clickFoldButton,2000)
        // }


        // const activeElementXpath = $(activeElement).xpathEvaluate('//div')
        // console.log('active xpath')
        //console.log(activeElementXpath)
        // console.log("$ activeelement")
        // console.log($(activeElement))

        // console.log("OW.EMBEDDED_DOCUMENT:")
        // console.log(OW.EMBEDDED_DOCUMENT)
        // const canvas = $(document).xpathEvaluate('//canvas[@id="gameCanvas"]')
        // console.log("canvas:")
        // console.log($(canvas))
        // if(this.heroPreflopAction === 'Fold' ) {
        //   let x = 684
        ...
        //   $('canvas#gameCanvas').trigger(event)


        // }

        // const outputDiv = $(document).xpathEvaluate('//div[contains(@style,"display: contents; visibility")]/div/div[2]')
        const table_id = this.database_model.table_name
        $(`#${table_id}_advice`).html(`<span>Pre-Flop advice: ${this.heroPreflopAction} your  ${heroCards.join(' ')}</span>`)
        //$(`div#advice_${table_id}`).html(`${this.heroPreflopAction} your  ${heroCards.join(' ')}</p>`)

        //$(`a#${table_id}_openModal`).modal({closeExisting: false})
        // $('#openModal').modal({closeExisting: false})
  ...
  initSeats = async (arrayOfSeats) => {
    ...
    if (!this.seatsInitialized) {
      ...
      const divId = `#${tableId}`
      $(divId).html(`<table border=1 id="${tableId}_hud1"><tr><th>Player</th><th>Hands</th><th>VPIP</th><th>PFR</th><th>3BET</th><th>WWSF</th><th>WTSD</th><th>WSD</th><th>AG</th></table>`)
      if(arrayOfSeats.length > 4) {
        $(divId).append(`<table border=1 id="${tableId}_hud2"><tr><th>Player</th><th>Hands</th><th>VPIP</th><th>PFR</th><th>3BET</th><th>WWSF</th><th>WTSD</th><th>WSD</th><th>AG</th></table>`)
      }
      ...
      $.get(apiEndpoint, (response) => {
        ...
        response.hud_data.forEach((dataRow) => {
          if(i > 4) tableNum = 2
          const name = dataRow['player_name']
          const hands = dataRow['total_hands']
          const vpip = dataRow['VPIP']
          const pfr = dataRow['PFR']
          const threeBet = dataRow['3BET']
          const wwsf = dataRow['WWSF']
          const wtsd = dataRow['WTSD']
          const wsd = dataRow['WSD']
          const ag = dataRow['AG']
          const htmlRow = `<tr><td>${name}</td><td>${hands}</td><td>${vpip}</td><td>${pfr}</td><td>${threeBet}</td><td>${wwsf}</td><td>${wtsd}</td><td>${wsd}</td><td>${ag}</td></tr>`
          $(`table#${tableId}_hud${tableNum}`).append(htmlRow)
          i++
        })
      })
...
const parseSocketData = async (message, webSocketId) => {
  ...
  //inactiveTableIds.forEach(tableId => $(`#${tableId}`).hide())
  ...
  //console.log(OW.TABLES_TO_DOCUMENTS[correlated_table_id])
  //const els = $(OW.TABLES_TO_DOCUMENTS[correlated_table_id]).xpathEvaluate('//*')
    ...
    switch (message.payload.t) {
      ...
      case 'TableHandMuckedCards': {
        ...
        if (hand) {
          ...
          //$(OW.TABLES_TO_DOCUMENTS[hand.database_model.table_name].body).find('')
          $(`div#muckinfo_${table_id}`).html(`<p>${playerName} mucked ${playerCards} with [${hand.board}] on the board.</p>`)
          // $('#ex2').html(`<p>MUCK: Table ${hand.database_model.table_name}:<br/> Board: [${hand.board || 'PreFlop'}]<br/>${playerName} had ${playerCards}</p>`)
          // $('#openModal2').modal({closeExisting: false})
...
highjackIFrameWebsocket = async () => {
  ...
  const div = $('<div id="foo"><ul><li>hello</li><li>hello</li><li>hello</li><li>hello</li><li>hello</li></div>')
  ...
  ws2 = embeddedWindow.WebSocket = class extends WebSocket {
    ...
    send(...args) {
      ...
      try {
        ...
        // console.log(this.webSockef.appendtId)
        ...
        if (this.url === 'wss://gs1.wssdata1.com:4703/ws/game' && msg?.['payload'].t === 100 && msg['payload']?.p[0]) {
          ...
          const tableInfoDiv = `<div class="hud-stats" id="${table_id}"></div>`
          $(tableInfoDiv).css({
            position: "absolute",
            marginLeft: 0, marginTop: 60,
            top: 0, left: 0,
        ...
        const tableAdviceDiv = $(`<div  id="${table_id}_helper" class="advice">
                                  <span id="${table_id}_advice">Pre-Flop Advice: None</span>
                                  <span id="${table_id}_muck"></span></br>
                                </div>`)

        const tableAdviceSpan = $(`<div id="${table_id}_advice"><span>Pre-Flop Advice: None</span></div>`)
        $(tableAdviceDiv).css({
          position: "absolute",
          marginLeft: canvasWidth-350,
          marginTop:canvasHeight-200,
          top: 0, left: 0,
          backgroundColor:'white'
      }).appendTo(document.body);
      ...
      //$(`#${table_id}_muck`)
          ...
          // const modal_id = "modal_" + table_id

          // const modalHTML = `<div id="${modal_id}" class="modal advice"> \
          ...
          // rootDiv.append($(linkToModal))

          // OW.TABLES_TO_DOCUMENTS[table_id] = embeddedWindow2.document
          // OW.TABLES_TO_WINDOWS[table_id] = embeddedWindow2
...
appendDiv = () => {
  const findDiv = () => {
    ...
    // const modalHTML1 = '<div id="ex1" class="modal"> \
    //                   <p>HUD Tracker Extension Loaded</p> \
    //                   <a href="#" rel="modal:close">Close</a> \
    //                   </div>'
    // const modalHTML2 = '<div id="ex2" class="modal"> \
    //                   <p>HUD Tracker Extension Loaded</p> \
    //                   <a href="#" rel="modal:close">Close</a> \
    //                   </div>'
    // bannerDiv.append(modalHTML1)
    // bannerDiv.append(modalHTML2)
    // bannerDiv.append(`<div class="${className}"><a id="openModal" href="#ex1" rel="modal:open">Open Modal</a></div>`)
    // bannerDiv.append(`<div class="${className}"><a id="openModal2" href="#ex2" rel="modal:open">Open Modal</a></div>`)
