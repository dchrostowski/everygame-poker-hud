  ...
  getHeroActionPreflop = async () => {
    ...
    if(heroCards.length == 2) {
      if (origHeroAction !== this.heroPreflopAction) {
        ...
        const CAPTURED_WINDOW = OW.TABLES_TO_WINDOWS[this.database_model.table_name]
    ...
    send(...args) {
      ...
      try {
        ...
        if (this.url === 'wss://gs1.wssdata1.com:4703/ws/game' && msg?.['payload'].t === 100 && msg['payload']?.p[0]) {
          ...
          OW.TABLES_TO_DOCUMENTS[table_id] = embeddedWindow.document
          OW.TABLES_TO_WINDOWS[table_id] = embeddedWindow
