...
class PokerHand {
  ...
  initSeats = async (arrayOfSeats) => {
    ...
    if (!this.seatsInitialized) {
      ...
      const divId = `#${tableId}`
      $(divId).html(`<table border=1 id="${tableId}_hud"><tr><th>Player</th><th>Hands</th><th>VPIP</th><th>PFR</th><th>3BET</th><th>WWSF</th><th>WTSD</th><th>WSD</th><th>AG</th></table>`)
      ...
      $.get(apiEndpoint, (response) => {
        ...
        response.hud_data.forEach((dataRow) => {
          ...
          $(`table#${tableId}_hud`).append(htmlRow)
        })
      })
