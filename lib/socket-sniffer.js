  ...
  getHeroActionPreflop = async () => {
    ...
    if(heroCards.length == 2) {
      if (origHeroAction !== this.heroPreflopAction) {
        ...
        $(`#hud_advice_${table_id}`).html(`<span class="hud-pre-flop-advice">Advice: ${this.heroPreflopAction} your  ${heroCards.join(' ')}</span>`)
  ...
  initSeats = async (arrayOfSeats) => {
    ...
    if (!this.seatsInitialized) {
      ...
      $.get(apiEndpoint, (response) => {
        $(divId).html(`<table class="hud hud-stats-table" id="hud_stats_table_${tableId}"><tr><th>Player</th><th>Hands</th><th>VPIP</th><th>PFR</th><th>3BET</th><th>WWSF</th><th>WTSD</th><th>WSD</th><th>AG</th></table>`)
        $(`table#hud_stats_table_${tableId}`).show()
        console.log(response)
        let i = 1

        response.hud_data.forEach((dataRow) => {
          let name = dataRow['player_name']
          if(name.length > 7) {
      ...
      arrayOfSeats
        ...
        .forEach(async (seatData) => {
          ...
          if(player.name === this.heroName && player.seat > 4 ) {
...
const parseSocketData = async (message, webSocketId) => {
  ...
  //const tableWidth = $(`table#hud_stats_table_${activeTableId}`).width()
  //$(`#hud_${activeTableId}`).css({width: tableWidth})
    ...
    switch (message.payload.t) {
      ...
      case 'TableHandMuckedCards': {
        ...
        if (hand) {
          ...
          if(OW.HERO_NAME === 'cornbl4ster') {
            $(`div#hud_muck_${table_id}`).html(`<span class="hud-muck-advice">${playerName} mucked ${playerCards} with [${hand.board}] on the board.</span>`)
          }
          ...
          <div class="hud hud-main" id="hud_${table_id}">
            <div class="hud hud-header">
              ...
              <div class="hud hud-header-center" id="hud_header_center_${table_id}">
                <span class="hud hud-title">HUD</span>
              </div>
