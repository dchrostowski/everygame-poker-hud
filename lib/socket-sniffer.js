...
class PokerHand {
    ...
    dealerMessage.forEach(val => {
      ...
      if(ohhActionMap.hasOwnProperty(dealerMessage[0])) {
        ...
        if(playerAction === 'Shows Cards') {
          console.log("SHOW CARDS HAPPENED, check street")
          const [dmc,playerName,rawCards] = dealerMessage
          this.playerCards[playerName] = rawCards.trim().split(' ')
          if(this.showdownOccurred) {

          }
          else {
            console.log("SHOWDOWN DID NOT OCCUR")
          }
        ...
        if(['39','18','19','7','6','5','21','4','10','4','11','10'].includes(dealerMessageCode.toString())) {


          if(!['11','4','9','10'].includes(dealerMessageCode.toString())) {
  ...
  initSeats = async (arrayOfSeats) => {
    ...
    if (!this.seatsInitialized) {
      arrayOfSeats
      .fitler(seatData => !['Unavailable','Empty'].includes(seatData['seat_state']))
    ...
    switch (message.payload.t) {
      ...
      case 'TableStartShowDown': {
        ...
        if (hand) {
          ...
          if(seatsInvolved.length > 1) hand.showdownOcurred = true
      ...
      case 'TableSeatState': {
        ...
        if (hand) {
          ...
          console.log("max seats: " + handMaxSeats)
