...
class PokerHand {
  ...
  getSeatByPlayerName = async (playerName) => {
    console.log("this.databasemodel.seats")
    console.log(this.database_model['seats'])
    for(let i=0; i<this.database_model['seats'].length; i++) {
      console.log('seat loop')
      const seat = this.database_model['seats'][i]
      console.log("seat is ")
      console.log(seat)
  ...
  // updateFromTableSeatSate = async(playerSeatState) => {
  ...
  //   if(player.last_action === 'PostAnte') {
  //     this.database_model['big_blind_amount'] = parseFloat(player.last_action_amount)
  //   }
  ...
  updateFromDealerChat = async (dealerMessage) => {
      ...
      if(!this.gameEvents.hasOwnProperty(this.street)) {
        round = new Round(this.street,this.board)
      }
      else {
        round = this.gameEvents[this.street]
      }
      ...
      if(ohhActionMap.hasOwnProperty(dealerMessage[0])) {
        ...
        if(['39','18','19','7','6','5','21'].includes(dealerMessageCode.toString())) {
          amount = dealerMessage[3]
          const playerAction = ohhActionMap[dealerMessageCode.toString()]

          const seat = this.database_model['seats'][seatNumber]
          const action = new Action(seatNumber,playerName,playerAction,amount,null)
          if(playerAction === 'Post SB') {

          }
          round.addAction(action)
          this.gameEvents[this.street] = round


          if(dealerMessageCode === 9) {

            console.log("LOOK FOR CARDS")
