...
class PokerHand {
  ...
  getPlayerSeatByName = async (playerName) => {
    ...
    // console.log('seats1')
    // console.log(JSON.stringify(seats1,undefined,4))
    // console.log('seats2')
    // console.log(JSON.stringify(seats2,undefined,4))
  ...
  initSeats = async (arrayOfSeats) => {
    ...
    if (!this.seatsInitialized) {
      arrayOfSeats.forEach(async (seatData) => {
    ...
    switch (message.payload.t) {
      ...
      case 'TableSeatState': {
        ...
        if (hand) {
          ...
          seatUpdateData.forEach(async (seat) => {
